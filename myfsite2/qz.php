...
$rs=mysql_query("select * from subject");
 while($row=mysql_fetch_row($rs)){
  ...
  echo '<input type="button" width="100%" class="btn btn-primary btn-block " disabled value='.$row[1] .'><br>'; 
if($subid==0)
...
echo '<input type="button" class="btn btn-primary" disabled value='.$row[2] .'>';  
if($_SESSION[tid]==0)
...
elseif(isset($subid)&&isset($_SESSION[tid]))
  echo " <input type=submit name=submit class='btn btn-danger' disabled value='Close Exam'></a>";
...
{ 
    ...
    else if($submit=='Get Result' && isset($ans))
    {
       $rs=mysql_query("select * from question where test_id=$tid",$cn) or die(mysql_error());
        ...
        echo "<h1><font color=gray size=4 align=center> <b> </b></h1>";
        echo '<div class="table-responsive">
<table class="table">
<tr class="info"><th align=center>RESULT</th></tr>
</table>
<table>
<tr class="info"><td>Candidate Name</td><td>'. $_SESSION['uname'] .'</td></tr>
<tr class="info"><td>Date</td><td>'.Date("d/m/y").'</td></tr>
<tr class="info"><td>Total Question</td><td>'.$_SESSION[qn].'</td></tr>
<tr class="info"><td>True Answer</td><td>'.$_SESSION[trueans].'</td></tr>
<tr class="info"><td>Wrong Answer</td><td>'.$w.'</td></tr>
<tr class="info"><td></td><td></td></tr>
